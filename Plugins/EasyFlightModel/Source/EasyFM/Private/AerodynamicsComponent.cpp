...
/*inline*/ float UAerodynamicsComponent::BlendInput(float low, float high) const {
	return FMath::Lerp(FMath::Clamp(low, -1.0f, 1.0f), FMath::Sign(high), FMath::Min(FMath::Abs(high), 1.0f));
};

/*inline*/ float UAerodynamicsComponent::Limiter(float input, float limitedVar, float min, float max, float sensitivity) const {
	return FMath::Clamp(input + (limitedVar - FMath::Clamp(limitedVar, min, max))*sensitivity, -1.0f, 1.0f);
};

/*inline*/ float UAerodynamicsComponent::Transit(float input, float target, float upspeed, float downspeed, float DeltaTime) const {
...
/*inline*/ float UAerodynamicsComponent::GetCurveValue(const UCurveFloat* curve, float in, float deflt) const {
...
/*inline*/ float UAerodynamicsComponent::GetAlpha(const FTransform &Transform, FVector velocity) const {
	FVector fwdvec, upvec, rightvec;
	GetAxisVectors(Transform, fwdvec, upvec, rightvec);
...
/*inline*/ float UAerodynamicsComponent::GetSlip(const FTransform &Transform, FVector velocity) const {
	FVector fwdvec, upvec, rightvec;
	GetAxisVectors(Transform, fwdvec, upvec, rightvec);
...
/*inline*/ void UAerodynamicsComponent::GetAxisVectors(const FTransform &Transform, FVector &fwdvec, FVector &upvec, FVector &rightvec) const {
