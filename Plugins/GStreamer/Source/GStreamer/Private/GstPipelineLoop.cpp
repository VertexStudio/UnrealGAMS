...
class FGstPipelineLoop : public IGstPipeline
{
public:

	FGstPipelineLoop() {}
	~FGstPipelineLoop() { Shutdown(); }
...
IGstPipeline *IGstPipeline::CreateLoop()
{
	auto Obj = new FGstPipelineLoop();
	GST_LOG_DBG_A("GstPipeline: CreateInstance %p", Obj);
	return Obj;
}

void FGstPipelineLoop::Destroy()
{
...
static gboolean BusMessageFunc(GstBus*, GstMessage* Message, FGstPipelineLoop* Context) { return Context->OnBusMessage(Message); }
static void ThreadWorkerFunc(FGstPipelineLoop* Context) { Context->WorkerLoop(); }

bool FGstPipelineLoop::Init(const char* Name, const char* Config)
{
...
void FGstPipelineLoop::Shutdown()
{
	Stop();
...
void FGstPipelineLoop::WorkerLoop()
{
...
gboolean FGstPipelineLoop::OnBusMessage(GstMessage* Message)
{
